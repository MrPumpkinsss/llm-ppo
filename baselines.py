...
def dp_partition(
    ...
    # Sort devices by compute power descending
    order = sorted(range(num_devices), key=lambda d: devices.compute_power[d], reverse=True)
    # Map: sorted_position -> original_device_id
    # inv_order[original_device_id] = sorted_position
    inv_order = {orig: pos for pos, orig in enumerate(order)}

    # Run DP with sorted order
    raw_partition = _dp_for_ordered_devices(
        num_layers, order, devices, layers, tensor_size
    )

    # Convert back to original device indices
    return [inv_order[d] for d in raw_partition]
...
def _dp_for_ordered_devices(
    ...
    Same as dp_for_device_order but ordered_devices is already 0-indexed.
    """
    ...
    for k in range(K, 0, -1):
        split = dp_parent[k][remaining_layers]
        dev_idx = ordered_devices[k - 1]
        for i in range(split, remaining_layers):
            partition[i] = dev_idx
        remaining_layers = split
...
def dp_for_device_order(
    ...
    Same as dp_partition but uses the provided device order instead of 0,1,2,...
    ...
    for k in range(K, 0, -1):
        split = dp_parent[k][remaining_layers]
        dev = device_order[k - 1]
        for l in range(split, remaining_layers):
            partition[l] = dev
        remaining_layers = split
