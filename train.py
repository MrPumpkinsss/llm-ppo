...
class MetricsCallback(BaseCallback):
    def __init__(self, eval_env, eval_freq=2000, n_eval=30, verbose=1):
        ...
        if self.n_calls % self.eval_freq == 0:
            ...
            for ep in range(self.n_eval):
                ...
                while not done:
                    ...
                    obs, rew, term, trunc, info = self.eval_env.step(act)
                    ep_rew += rew
                    done = term or trunc
                if 'tpot' in info:
            ...
            if tpots:
                ...
                if self.stag > 8:
                    ...
                    self.model.ent_coef = max(0.005, self.model.ent_coef * 0.98)

                if self.verbose:
                    print(f"\n  [Eval@{self.num_timesteps}] mean={mt:.1f} min={np.min(tpots):.1f} "
                          f"best={self.best_tpot:.1f} ent={self.model.ent_coef:.4f} sw={np.mean(switches):.1f}")
        return True
...
def train(config=None):
    ...
    print("=" * 60)
    print("Training: 28 layers, sequential decision, dense reward")
    print("=" * 60)
    ...
    print(f"Obs: {train_env.observation_space.shape}, Act: {train_env.action_space}")
